        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '现场勘查名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.xianchangName" placeholder="现场勘查名称" clearable></el-input>
                     </el-form-item>
                         
                     <el-form-item :label="contents.inputTitle == 1 ? '现场勘查类型' : ''">
                        <el-select v-model="searchForm.xianchangTypes" placeholder="请选择现场勘查类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in xianchangTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xianchang','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chexianlipeixinxiguanlixitong/upload/xianchangMuBan.xls"
                        >批量导入现场勘查数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xianchang','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="xianchangUploadSuccess"
                                :on-error="xianchangUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('xianchang','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入现场勘查数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xianchang','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xianchang.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="现场勘查名称">
                        <template slot-scope="scope">
                            {{scope.row.xianchangName}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xianchangUuidNumber"
                                   header-align="center"
                                   label="现场勘查编号">
                        <template slot-scope="scope">
                            {{scope.row.xianchangUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="xianchangPhoto"
                                     header-align="center"
                                     width="200"
                                     label="现场勘查照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.xianchangPhoto">
                                <img :src="$base.url+scope.row.xianchangPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="现场勘查类型">
                        <template slot-scope="scope">
                            {{scope.row.xianchangValue}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="xianchangFile"
                                     header-align="center"
                                     width="200"
                                     label="文件">
                        <template slot-scope="scope">
                            <div v-if="scope.row.xianchangFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.xianchangFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('xianchang','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xianchang','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('xianchang','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //级联表下拉框搜索条件
    //当前表下拉框搜索条件
              xianchangTypesSelectSearch : [],
            form:{
                id : null,
                yonghuId : null,
                yuangongId : null,
                xianchangName : null,
                xianchangUuidNumber : null,
                xianchangPhoto : null,
                xianchangTypes : null,
                xianchangFile : null,
                xianchangContent : null,
                xianchangDelete : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                ...
                //本表字段
                     '现场勘查名称': "xianchangName",
                     '现场勘查编号': "xianchangUuidNumber",
                     '现场勘查照片': "xianchangPhoto",
                     '现场勘查类型': "xianchangValue",
                     '文件': "xianchangFile",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'xianchang',//当前表表名,
                        sumColum : 'xianchang_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'xianchang_name',//分组字符串字段
                        // types : 'xianchang_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.xianchangName!= '' && this.searchForm.xianchangName!= undefined) {
                    params['xianchangName'] = '%' + this.searchForm.xianchangName + '%'
                }
                         
                if (this.searchForm.xianchangTypes!= '' && this.searchForm.xianchangTypes!= undefined) {
                    params['xianchangTypes'] = this.searchForm.xianchangTypes
                }
                                        
                params['xianchangDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=xianchang_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.xianchangTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            xianchangUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xianchang/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
